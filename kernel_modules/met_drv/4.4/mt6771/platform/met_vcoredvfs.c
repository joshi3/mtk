...
#if 0 /* re-implement as above */
__weak char *governor_get_kicker_name(int id) { return "KIR_UNKNOWN"; }
__weak int vcorefs_get_opp_info_num(void) { return DEFAULT_INFO_NUM; }
__weak char **vcorefs_get_opp_info_name(void) { return default_info_name; }
__weak unsigned int *vcorefs_get_opp_info(void)
{
	int count = 0;

	opp_info[count++] = vcorefs_get_hw_opp();
	opp_info[count++] = vcorefs_get_curr_vcore(); /* uV */
	opp_info[count++] = vcorefs_get_curr_ddr(); /* kHz */

	return opp_info;
}

__weak int vcorefs_get_src_req_num(void) { return DEFAULT_SRC_NUM; }
__weak char **vcorefs_get_src_req_name(void) { return default_src_name; }
__weak unsigned int *vcorefs_get_src_req(void)
{
	src_req[0] = spm_vcorefs_get_MD_status();

	return src_req;
}

extern int __attribute__ ((weak)) vcorefs_enable_debug_isr(bool);
#endif
