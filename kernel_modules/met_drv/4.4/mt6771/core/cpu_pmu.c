...
static int cpupmu_create_subfs(struct kobject *parent)
{
	...
	if (cpu_pmu == NULL) {
		pr_debug("Failed to init CPU PMU HW!!\n");
		return -ENODEV;
	}
	...
	if (ret != 0) {
		pr_debug("Failed to create count in sysfs\n");
		goto out;
	}
	...
	for (i = 0; i < nr_counters; i++) {
		...
		if (ret != 0) {
			pr_debug("Failed to create event in sysfs\n");
			goto out;
		}

		ret = sysfs_create_file(pmu[i].kobj_cpu_pmu, &mode_attr.attr);
		if (ret != 0) {
			pr_debug("Failed to create mode in sysfs\n");
			goto out;
		}

		ret = sysfs_create_file(pmu[i].kobj_cpu_pmu, &freq_attr.attr);
		if (ret != 0) {
			pr_debug("Failed to create freq in sysfs\n");
			goto out;
		}
	}
