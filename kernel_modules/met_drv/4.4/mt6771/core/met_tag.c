...
static noinline int tracing_mark_write(int type, unsigned int class_id,
...
static int (*show_bw_limiter)(void);
int met_show_bw_limiter(void)
{
	if (show_bw_limiter != NULL)
		show_bw_limiter();
	return 0;
}
...
static int (*show_clk_tree)(const char *name, unsigned int addr, unsigned int status);
int met_show_clk_tree(const char *name, unsigned int addr, unsigned int status)
{
	if (show_clk_tree != NULL)
		show_clk_tree(name, addr, status);
	return 0;
}
...
static long met_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MTAG_CMD_USRDATA:
		if (pUserCmd->slen > MET_MAX_STRLENGTH)
			return -EFAULT;
...
#ifdef	CONFIG_COMPAT
struct compat_mtag_cmd_t {
	compat_uint_t class_id;
	compat_uint_t value;
	compat_uint_t slen;
...
/* redefine COMPAT ioctl command */
#define COMPAT_MTAG_CMD_START		_IOW(MTAG_IOC_MAGIC, TYPE_START, struct compat_mtag_cmd_t)
#define COMPAT_MTAG_CMD_END		_IOW(MTAG_IOC_MAGIC, TYPE_END, struct compat_mtag_cmd_t)
#define COMPAT_MTAG_CMD_ONESHOT		_IOW(MTAG_IOC_MAGIC, TYPE_ONESHOT, struct compat_mtag_cmd_t)
#define COMPAT_MTAG_CMD_ENABLE		_IOW(MTAG_IOC_MAGIC, TYPE_ENABLE, compat_int_t)
#define COMPAT_MTAG_CMD_DISABLE		_IOW(MTAG_IOC_MAGIC, TYPE_DISABLE, compat_int_t)
#define COMPAT_MTAG_CMD_REC_SET		_IOW(MTAG_IOC_MAGIC, TYPE_REC_SET, compat_int_t)
#define COMPAT_MTAG_CMD_DUMP		_IOW(MTAG_IOC_MAGIC, TYPE_DUMP, struct compat_mtag_cmd_t)
#define COMPAT_MTAG_CMD_DUMP_SIZE	_IOWR(MTAG_IOC_MAGIC, TYPE_DUMP_SIZE, compat_int_t)
#define COMPAT_MTAG_CMD_DUMP_SAVE	_IOW(MTAG_IOC_MAGIC, TYPE_DUMP_SAVE, struct compat_mtag_cmd_t)
#define COMPAT_MTAG_CMD_USRDATA		_IOW(MTAG_IOC_MAGIC, TYPE_USRDATA, struct compat_mtag_cmd_t)
#define COMPAT_MTAG_CMD_DUMP_AGAIN	_IOW(MTAG_IOC_MAGIC, TYPE_DUMP_AGAIN, compat_uptr_t)

/*
 * compat_sync_no == compat_sync_no_plus1: no in a command pair
 * compat_sync_no+1 == compat_sync_no_plus1: in a command pair
 */
static unsigned int compat_sync_cmd;
static struct compat_mtag_cmd_t compat_sync_data;
static int compat_sync_no;
static int compat_sync_no_plus1;

static long compat_met_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	/* mtag_cmd_t cblk; */
	struct compat_mtag_cmd_t *pUserCmd;
	char str[MET_MAX_STRLENGTH];
	int ret = 0;
	...
	/* Hanlde command without copying data from user space */
	if (cmd == COMPAT_MTAG_CMD_ENABLE)
		return met_tag_enable((unsigned int)arg);
	else if (cmd == COMPAT_MTAG_CMD_DISABLE)
		return met_tag_disable((unsigned int)arg);
	else if (cmd == COMPAT_MTAG_CMD_REC_SET) {
		if (arg)
			tracing_on();
		else
			tracing_off();
		return 0;
	} else if (cmd == COMPAT_MTAG_CMD_DUMP_SIZE) {
		ret = met_set_dump_buffer((int)arg);
		return (long)ret;
	}
	...
	if (ret)
		return -EFAULT;
	pUserCmd = (struct compat_mtag_cmd_t *) arg;

	switch (cmd) {
	case COMPAT_MTAG_CMD_START:
		ret = met_tag_start(pUserCmd->class_id, pUserCmd->tname);
		break;
	case COMPAT_MTAG_CMD_END:
		ret = met_tag_end(pUserCmd->class_id, pUserCmd->tname);
		break;
	case COMPAT_MTAG_CMD_ONESHOT:
		ret = met_tag_oneshot(pUserCmd->class_id, pUserCmd->tname, pUserCmd->value);
		break;
	case COMPAT_MTAG_CMD_DUMP:
		if (compat_sync_no != compat_sync_no_plus1) {
			compat_sync_no = compat_sync_no_plus1;	/* keep race */
			return -EFAULT;
		}
		compat_sync_cmd = COMPAT_MTAG_CMD_DUMP;
		memcpy(&compat_sync_data, pUserCmd, sizeof(compat_sync_data));
		compat_sync_no_plus1 = compat_sync_no + 1;
		return -EAGAIN;
	case COMPAT_MTAG_CMD_DUMP_SAVE:
		ret = met_save_dump_buffer(pUserCmd->tname);
		break;
	case COMPAT_MTAG_CMD_USRDATA:
		if (compat_sync_no != compat_sync_no_plus1) {
			compat_sync_no = compat_sync_no_plus1;	/* keep race */
			return -EFAULT;
		}
		compat_sync_cmd = COMPAT_MTAG_CMD_USRDATA;
		memcpy(&compat_sync_data, pUserCmd, sizeof(compat_sync_data));
		compat_sync_no_plus1 = compat_sync_no + 1;
		return -EAGAIN;
	case COMPAT_MTAG_CMD_DUMP_AGAIN:
		if (compat_sync_no + 1 != compat_sync_no_plus1) {
			compat_sync_no = compat_sync_no_plus1;	/* reset sync no */
			return -EFAULT;
		}
		compat_sync_no = compat_sync_no_plus1;
		switch (compat_sync_cmd) {
		case COMPAT_MTAG_CMD_DUMP:
			ret = !access_ok(VERIFY_READ, (void __user *)arg, compat_sync_data.size);
			if (ret)
				return -EFAULT;
			ret =
			    met_tag_dump(compat_sync_data.class_id, compat_sync_data.tname,
					 (void *)arg, compat_sync_data.size);
			break;
		case COMPAT_MTAG_CMD_USRDATA:
			if (compat_sync_data.slen > MET_MAX_STRLENGTH)
				return -EFAULT;
			ret = copy_from_user(str, (void __user *)arg, compat_sync_data.slen);
			if (ret)
				return -EFAULT;
			str[sizeof(str) - 1] = 0;
			met_tag_userdata(str);
...
#ifdef	CONFIG_COMPAT
	fops->compat_ioctl = compat_met_ioctl;
#endif
	fops->read = met_read;
