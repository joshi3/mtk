...
INT32 mtk_wcn_consys_jtag_set_for_mcu(VOID)
{
	...
	if (gJtagCtrl) {
		...
		case 1:
			...
			WMT_PLAT_INFO_FUNC("WCN jtag set for DSP start...\n");
#if 1
			/* PAD AUX Function Selection */
...
static VOID consys_set_if_pinmux(MTK_WCN_BOOL enable)
{
	...
	consys_if_pinmux_reg_base = ioremap_nocache(CONSYS_IF_PINMUX_REG_BASE, 0x100);
...
#ifdef CONFIG_OF		/*use DT */
	if (enable) {
		/*3.assert CONNSYS CPU SW reset  0x10211018 "[12]=1'b1  [31:24]=8'h88 (key)" */
...
{
#ifdef CONFIG_OF		/*use DT */
	/*turn on SPM clock gating enable PWRON_CONFG_EN  0x10a00000  32'h0b160001 */
...
static INT32 consys_hw_power_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
...
#ifdef CONFIG_OF		/*use DT */
		/*2.write conn_top1_pwr_on=1, power on conn_top1 0x10a00324 [2]  1'b1 */
		...
		/*2.write conn_top1_pwr_on_s=1, power on conn_top1 0x10a00324 [3]  1'b1 */
		CONSYS_REG_WRITE(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET,
				 CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET) |
				 CONSYS_SPM_PWR_ON_S_BIT);
		/*3.read conn_top1_pwr_on_ack =1, power on ack ready 0x10a00190 [10] */
		value = CONSYS_PWR_ON_ACK_BIT &
			CONSYS_REG_READ(conn_reg.spm_base + CONSYS_PWR_CONN_ACK_OFFSET);
		while (value == 0)
			value = CONSYS_PWR_ON_ACK_BIT &
				CONSYS_REG_READ(conn_reg.spm_base + CONSYS_PWR_CONN_ACK_OFFSET);
		/*3.read conn_top1_pwr_on_ack_s =1, power on ack ready 0x10a00194 [10] */
		value = CONSYS_PWR_ON_ACK_S_BIT &
			CONSYS_REG_READ(conn_reg.spm_base + CONSYS_PWR_CONN_ACK_S_OFFSET);
		while (value == 0)
			value = CONSYS_PWR_ON_ACK_S_BIT &
				CONSYS_REG_READ(conn_reg.spm_base + CONSYS_PWR_CONN_ACK_S_OFFSET);
		/*6.write conn_clk_dis=0, enable connsys clock 0x10a00324 [4]  1'b0 */
		CONSYS_REG_WRITE(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET,
				 CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET) &
				 ~CONSYS_CLK_CTRL_BIT);
		/*7.wait 1us    */
		udelay(1);
		/*9.release connsys ISO, conn_top1_iso_en=0 0x10a00324 [1]  1'b0 */
		CONSYS_REG_WRITE(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET,
				 CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET) &
				 ~CONSYS_SPM_PWR_ISO_S_BIT);
		/*10.release SW reset of connsys, conn_ap_sw_rst_b=1  0x10a00324[0]   1'b1 */
		CONSYS_REG_WRITE(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET,
				 CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET) |
				 CONSYS_SPM_PWR_RST_BIT);
		/*read conn_top2_pwr_on_ack =1, power on ack ready 0x10a00324 [28] */
		value = CONSYS_TOP2_PWR_ON_ACK_BIT &
			CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET);
		while (value == 0)
			value = CONSYS_TOP2_PWR_ON_ACK_BIT &
				CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET);
		/*read conn_top2_pwr_on_ack_s =1, power on ack ready 0x10a00324 [29] */
		value = CONSYS_TOP2_PWR_ON_ACK_S_BIT &
			CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET);
		while (value == 0)
			value = CONSYS_TOP2_PWR_ON_ACK_S_BIT &
				CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET);
		/*disable AXI BUS protect 0x10000220[10] [19] */
		...
		value = CONSYS_PWR_ON_ACK_BIT & CONSYS_REG_READ(CONSYS_PWR_CONN_ACK_REG);
		while (value == 0)
			value = CONSYS_PWR_ON_ACK_BIT & CONSYS_REG_READ(CONSYS_PWR_CONN_ACK_REG);
		...
		value = CONSYS_PWR_CONN_ACK_S_BIT & CONSYS_REG_READ(CONSYS_PWR_CONN_ACK_S_REG);
		while (value == 0)
			value = CONSYS_PWR_CONN_ACK_S_BIT & CONSYS_REG_READ(CONSYS_PWR_CONN_ACK_S_REG);
		...
		value = CONSYS_REG_READ(CONSYS_TOPAXI_PROT_STA1) & CONSYS_PROT_MASK;
		while (value)
			value = CONSYS_REG_READ(CONSYS_TOPAXI_PROT_STA1) & CONSYS_PROT_MASK;
#endif /* CONFIG_OF */
		...
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_TOPAXI_PROT_EN_OFFSET,
				CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_TOPAXI_PROT_EN_OFFSET) |
				CONSYS_PROT_MASK);
		/*assert SW reset of connsys, conn_ap_sw_rst_b=0  0x10a00324[0] 1'b0 */
		CONSYS_REG_WRITE(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET,
				 CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET) &
				 ~CONSYS_SPM_PWR_RST_BIT);
		/*release connsys ISO, conn_top1_iso_en=1  0x10a00324 [1]  1'b1 */
		CONSYS_REG_WRITE(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET,
				 CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET) |
				 CONSYS_SPM_PWR_ISO_S_BIT);
		/*write conn_clk_dis=1, disable connsys clock  0x10a00324 [4]  1'b1 */
		CONSYS_REG_WRITE(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET,
				 CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET) |
				 CONSYS_CLK_CTRL_BIT);
		/*wait 1us      */
		udelay(1);
		/*write conn_top1_pwr_on=0, power off conn_top1 0x10a00324 [3:2] 2'b00 */
		CONSYS_REG_WRITE(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET,
				 CONSYS_REG_READ(conn_reg.spm_base + CONSYS_TOP1_PWR_CTRL_OFFSET) &
...
#else
	/*12.poll CONNSYS CHIP ID until 6758 is returned 0x18070008 32'h0688 */
	while (retry-- > 0) {
...
#if defined(CONFIG_MTK_PMIC_CHIP_MT6355)
		WMT_PLAT_INFO_FUNC("VCN18 SW mode before power on:0x%x\n",
				KERNEL_pmic_get_register_value(PMIC_RG_LDO_VCN18_SW_OP_EN));
		WMT_PLAT_INFO_FUNC("VCN18 before power on:0x%x\n",
				KERNEL_pmic_get_register_value(PMIC_RG_LDO_VCN18_EN));
...
#if defined(CONFIG_MTK_PMIC_CHIP_MT6355)
		WMT_PLAT_INFO_FUNC("VCN18 SW mode after power on:0x%x\n",
				KERNEL_pmic_get_register_value(PMIC_RG_LDO_VCN18_SW_OP_EN));
		WMT_PLAT_INFO_FUNC("VCN18 after power on:0x%x\n",
				KERNEL_pmic_get_register_value(PMIC_RG_LDO_VCN18_EN));

		WMT_PLAT_INFO_FUNC("VCN33 SW mode before power on:0x%x\n",
				KERNEL_pmic_get_register_value(PMIC_RG_LDO_VCN33_SW_OP_EN));
		WMT_PLAT_INFO_FUNC("VCN33 before power on:0x%x\n",
				KERNEL_pmic_get_register_value(PMIC_RG_LDO_VCN33_EN_BT));
...
#if defined(CONFIG_MTK_PMIC_CHIP_MT6355)
		WMT_PLAT_INFO_FUNC("VCN33 SW mode after power on:0x%x\n",
				KERNEL_pmic_get_register_value(PMIC_RG_LDO_VCN33_SW_OP_EN));
		WMT_PLAT_INFO_FUNC("VCN33 after power on:0x%x\n",
				KERNEL_pmic_get_register_value(PMIC_RG_LDO_VCN33_EN_BT));
#endif
	} else {
		if (reg_VCN33_BT)
