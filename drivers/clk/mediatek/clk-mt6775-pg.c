...
#define IPU_CG			(clk_ipusys_conn_base + 0x0000)
#define IPU_CG_CLR		(clk_ipusys_conn_base + 0x0008)
...
static int IPU_VCORE_SHUTDOWN_sys_enable_op(struct subsys *sys)
{
	...
	return spm_mtcmos_ctrl_ipu_vcore_shut_down(STA_POWER_ON);
}
...
struct mtk_power_gate scp_clks[] __initdata = {
	...
	PGATE2(SCP_SYS_IPU_CORE0_SHUTDOWN, pg_ipu_core0_shutdown, NULL, ipu_if_sel, dsp_sel, SYS_IPU_CORE0_SHUTDOWN),
	PGATE2(SCP_SYS_IPU_CORE1_SHUTDOWN, pg_ipu_core1_shutdown, NULL, ipu_if_sel, dsp_sel, SYS_IPU_CORE1_SHUTDOWN),
	PGATE2(SCP_SYS_IPU_CORE2_SHUTDOWN, pg_ipu_core2_shutdown, NULL, ipu_if_sel, dsp_sel, SYS_IPU_CORE2_SHUTDOWN),
	PGATE2(SCP_SYS_IPU_CORE0_SLEEP, pg_ipu_core0_sleep, NULL, ipu_if_sel, dsp_sel, SYS_IPU_CORE0_SLEEP),
	PGATE2(SCP_SYS_IPU_CORE1_SLEEP, pg_ipu_core1_sleep, NULL, ipu_if_sel, dsp_sel, SYS_IPU_CORE1_SLEEP),
	PGATE2(SCP_SYS_IPU_CORE2_SLEEP, pg_ipu_core2_sleep, NULL, ipu_if_sel, dsp_sel, SYS_IPU_CORE2_SLEEP),
};
...
{
	if (on)
		clk_writel(MM_CG_CLR0, 0x04000019);
	else {
...
void ven_mtcmos_patch(void)
{
	...
	clk_writel(MM_CG_CLR0, 0x00000039);
}
...
void vde_mtcmos_patch(void)
{
	...
	clk_writel(VDEC_LARB1_SET, 0x00000001);
	clk_writel(MM_CG_CLR0, 0x000000059);
}
...
void isp_mtcmos_patch(int on)
{
	if (on)
		...
		clk_writel(MM_CG_CLR0, 0x00000099);/*prevent bus ack fail*/
}
...
void cam_mtcmos_patch(int on)
{
	if (on)
		...
		clk_writel(MM_CG_CLR0, 0x00000119);/*prevent bus ack fail*/
}
...
void ipu_mtcmos_patch(int on)
{
	if (on)
		...
		clk_writel(MM_CG_CLR0, 0x00000219);/*prevent bus ack fail*/
}
...
void subsys_if_on(void)
{
	...
	if ((sta & (1U << 16)) && (sta_s & (1U << 16))) {
		pr_notice("suspend warning: SYS_IPU is on!!!\n");
		pr_debug("IPU_CG(%08x): %08x\r\n",
			spm_read(IPU_PWR_CON),		/* todo : check again*/
			spm_read(IPU_CG));
		ret++;
	}
	...
	if ((sta & (1U << 20)) && (sta_s & (1U << 20))) {
		...
		pr_debug("IPU Core0_CG(%08x): %08x\r\n",
			...
			spm_read(IPU_CG));	/* todo : change to core0 address */
		ret++;
	}
	...
	if ((sta & (1U << 21)) && (sta_s & (1U << 21))) {
		...
		pr_debug("IPU Core1_CG(%08x): %08x\r\n",
			...
			spm_read(IPU_CG));	/* todo : change to core1 address */
		ret++;
	}
	...
	if ((sta & (1U << 22)) && (sta_s & (1U << 22))) {
		...
		pr_debug("IPU Core2_CG(%08x): %08x\r\n",
			...
			spm_read(IPU_CG));	/* todo : change to core2 address */
		ret++;
	}
...
void mtcmos_force_off(void)
{
	...
	spm_mtcmos_ctrl_ipu_shut_down(STA_POWER_DOWN);		/* yy: check this again */
