...
int spm_vcorefs_pwarp_cmd(void)
{
	...
	mt_spm_pmic_wrap_set_cmd(PMIC_WRAP_PHASE_ALLINONE, CMD_8, get_vcore_opp_volt(VCORE_OPP_2)); /* 0.625 */
	mt_spm_pmic_wrap_set_cmd(PMIC_WRAP_PHASE_ALLINONE, CMD_9, get_vcore_opp_volt(VCORE_OPP_1)); /* 0.7 */
	mt_spm_pmic_wrap_set_cmd(PMIC_WRAP_PHASE_ALLINONE, CMD_10, get_vcore_opp_volt(VCORE_OPP_0)); /* 0.8 */
	...
	spm_d.u.vcorefs.vcore_level0 = get_vcore_opp_volt(VCORE_OPP_0);
	spm_d.u.vcorefs.vcore_level1 = get_vcore_opp_volt(VCORE_OPP_1);
	spm_d.u.vcorefs.vcore_level2 = get_vcore_opp_volt(VCORE_OPP_2);
...
void dvfsrc_set_vcore_request(unsigned int mask, unsigned int shift, unsigned int level)
{
	...
	val = (spm_read(DVFSRC_VCORE_REQUEST) & ~mask) | (level << shift);
...
{
	dvfsrc_set_vcore_request((0x3 << 30), 30, (level & 0x3));
}
...
void dvfsrc_init(void)
{
	...
	spm_write(DVFSRC_EMI_REQUEST3, 0x39393939);
	spm_write(DVFSRC_VCORE_REQUEST, 0x00390000);
	spm_write(DVFSRC_VCORE_REQUEST2, 0x39393939);
