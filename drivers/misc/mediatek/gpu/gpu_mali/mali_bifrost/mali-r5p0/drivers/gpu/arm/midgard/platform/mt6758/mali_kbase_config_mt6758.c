...
static int _mtk_pm_callback_power_on(void)
{
	...
	/*pr_info("[MALI] power on!, curFreqID(%d), vgpu_ref_count(%d)\n", g_curFreqID, g_vgpu_ref_count);*/
...
static void _mtk_pm_callback_power_off(void)
{
	...
	/*pr_info("[MALI] power off! vgpu_ref_count(%d)\n", g_vgpu_ref_count);*/
	mutex_unlock(&mtk_pm_callback_lock);
#endif
}
...
static int _mtk_pm_callback_power_resume(void)
{
	...
	mutex_lock(&mtk_pm_callback_lock);
	/*pr_info("[MALI] start power resume..\n");*/

	mt_gpufreq_voltage_enable_set(1);
	mtk_set_vgpu_power_on_flag(MTK_VGPU_POWER_ON);

	pr_info("[MALI] power resume!\n");
	mutex_unlock(&mtk_pm_callback_lock);
#endif
...
static void _mtk_pm_callback_power_suspend(void)
{
	...
	mutex_lock(&mtk_pm_callback_lock);
	/*pr_info("[MALI] start power suspend.. vgpu_ref_count(%d)\n", g_vgpu_ref_count);*/

	mtk_set_vgpu_power_on_flag(MTK_VGPU_POWER_OFF);
	mt_gpufreq_voltage_enable_set(0);
	mdelay(3);

	pr_info("[MALI] power suspend!\n");
	mutex_unlock(&mtk_pm_callback_lock);
#endif
}
...
int mtk_platform_init(struct platform_device *pdev, struct kbase_device *kbdev)
{
	...
	pr_info("xxxx clk_mfg0:%p\n", config->clk_mfg0);
	pr_info("xxxx clk_mfg1:%p\n", config->clk_mfg1);
	pr_info("xxxx clk_mfg2:%p\n", config->clk_mfg2);
	pr_info("xxxx clk_mfg3:%p\n", config->clk_mfg3);
	pr_info("xxxx clk_mfg_cg:%p\n", config->clk_mfg_cg);
